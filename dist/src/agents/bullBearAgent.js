...
exports.BullBearAgent = void 0;
class BullBearAgent {
    async run(market, evidence, rules) {
        const topNews = evidence.newsItems.slice(0, 3).map((n) => `${n.source}: ${n.title}`);
        const supportiveSignals = [];
        const cautionSignals = [];
        if ((market.spread ?? 0.05) <= 0.02)
            supportiveSignals.push("tight spread suggests executable pricing");
        if (market.liquidity >= 50000)
            supportiveSignals.push("strong liquidity reduces execution friction");
        if ((market.spread ?? 0.05) > 0.03)
            cautionSignals.push("wide spread may hide true executable odds");
        if (rules.resolutionRisk !== "low")
            cautionSignals.push(`resolution risk is ${rules.resolutionRisk}`);
        if (evidence.newsItems.length === 0)
            cautionSignals.push("no recent relevant news found");
        return {
...
exports.BullBearAgent = BullBearAgent;
