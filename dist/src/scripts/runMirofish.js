...
const mirofishSwarmAgent_1 = require("../agents/mirofishSwarmAgent");
...
void (async () => {
    ...
    const swarm = new mirofishSwarmAgent_1.MiroFishSwarmAgent(client);
    ...
    for (const record of seedRecords) {
        try {
            const seed = validation_1.seedPacketSchema.parse(record.seed);
            const mode = config_1.config.RUN_DEEP_MIROFISH ? "deep" : "light";
            ...
            if (!response.ok || !response.result) {
                ...
                await store.insertDecision(runId, seed.marketId, {
                    marketId: seed.marketId,
                    ...
                    confidence: "low"
                }, { approved: false, reason: "Phase 3 prediction-only mode" }, { status: "dry_run", message: "No execution in Phase 3" }, true);
            ...
            await store.insertDecision(runId, seed.marketId, {
                ...
                confidence: response.result.rawConfidenceScore >= 75
                    ? "high"
            ...
            if (config_1.config.TELEGRAM_VERBOSE) {
                await telegram.sendText(`MIROFISH OK\nMarket: ${seed.question}\nConfidence: ${response.result.rawConfidenceScore.toFixed(1)}%\nProbability: ${(response.result.rawProbability * 100).toFixed(1)}%`);
            }
        }
        catch (err) {
            failCount += 1;
            logger_1.logger.error({ err, marketId: record.marketId }, "Phase 3 processing failed");
            await store.insertMiroFishResult({
                runId,
                marketId: record.marketId,
                seedPacketId: record.id,
                mode: config_1.config.RUN_DEEP_MIROFISH ? "deep" : "light",
                agents: config_1.config.RUN_DEEP_MIROFISH ? config_1.config.MIROFISH_DEEP_AGENTS : config_1.config.MIROFISH_LIGHT_AGENTS,
                rounds: config_1.config.RUN_DEEP_MIROFISH ? config_1.config.MIROFISH_DEEP_ROUNDS : config_1.config.MIROFISH_LIGHT_ROUNDS,
    ...
    logger_1.logger.error({ err }, "Phase 3 run crashed");
    process.exit(1);
});
